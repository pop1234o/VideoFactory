...
public class ClassUtils {
    ...
    getObjectsWithInterface(Context context, Class<T> clazz, String path) {
        ...
        try {
            ...
            for (String className : classFileNames) {
                if (className != null && className.startsWith("com.tal.user")) {
                    continue;
                }
    ...
    public static List<String> getFileNameByPackageName(Context context, String packageName) throws PackageManager.NameNotFoundException, IOException {
        ...
        for (String path : getSourcePaths(context)) {
            ...
            try {
                dexfile = path.endsWith(EXTRACTED_SUFFIX) ? DexFile.loadDex(path, path + ".tmp", 0) : new DexFile(path);
                ...
                Log.e(TAG, "Scan map file in dex files made error.", e.getCause());
            } finally {
                if (null != dexfile) {
                    try {
                        dexfile.close();
